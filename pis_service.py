...
def get_epic_cycle_time_status(cycle_time: Optional[float]) -> str:
    ...
    Args:
        cycle_time: Average epic cycle time in days (can be None if no data)
    
    Returns:
        "green" if cycle_time <= 50
        "yellow" if 50 < cycle_time <= 70
        "red" if cycle_time > 70
...
async def get_average_epic_cycle_time(
    ...
    """
    Get average cycle time (in days) for completed epics in the last N months.
    
    Calculates the average cycle time for all epics that are:
    - Issue type = 'Epic'
    - Status category = 'Done'
    - Have a non-null cycle_time_days value
    - Were resolved (resolved_at) within the last N months
    - Optionally filtered by team name or group
    
    Args:
        months: Number of months to look back (default: 3, valid: 1-12)
        team_name: Optional team name or group name (if isGroup=true)
        isGroup: If true, team_name is treated as a group name
    
    Returns:
        JSON response with average epic cycle time, epic count, and metadata
    """
    try:
        ...
        # Add team filtering if team_name provided
        if team_names_list:
        ...
        # Extract results
        avg_cycle_time = float(row[0]) if row and row[0] is not None else None
        epic_count = int(row[1]) if row and row[1] is not None else 0
        ...
        # Build response data
        response_data = {
            "average_epic_cycle_time": round(avg_cycle_time, 2) if avg_cycle_time is not None else None,
        ...
        # Build message
        if epic_count > 0:
            message = f"Retrieved average epic cycle time: {response_data['average_epic_cycle_time']} days ({epic_count} epics, last {months} months)"
        else:
            message = f"No completed epics found with cycle time data for the last {months} months"
